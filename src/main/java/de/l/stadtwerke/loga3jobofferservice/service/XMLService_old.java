...
@Service
public class XMLService_old {
    @Autowired
    private StellenausschreibungRepository stellenausschreibungRepository;

    public List<Stellenausschreibung_old> parseJobs() {

        List<Stellenausschreibung_old> stellenListe = new ArrayList();
        try {
            DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
            DocumentBuilder builder = factory.newDocumentBuilder();
            File fXmlFile = new File("src/main/resources/logaTest_old.xml");
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if (node.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    Stellenausschreibung_old stelle = new Stellenausschreibung_old();
